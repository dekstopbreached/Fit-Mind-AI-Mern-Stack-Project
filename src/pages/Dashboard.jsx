...
export default function Dashboard() {
  ...
  const habitsForToday = useMemo(
    () => habits.filter((h) => !h.scheduledDays || h.scheduledDays.includes(todayDow)),
    [habits, todayDow]
  );
  ...
  const scheduledTodayCompletedCount = useMemo(
    () => habitsForToday.filter((h) => completedToday.has(String(h._id))).length,
    [habitsForToday, completedToday]
  );
  ...
  const todayProgress = habitsForToday.length
    ? Math.round((scheduledTodayCompletedCount / habitsForToday.length) * 100)
    : 0;
  ...
  const workoutVideos = [
    {
      day: "Monday",
      dow: 1,
      title: "Gym Motivational Song",
      id: "IHDeFXYMEUQ",
      description: "Science-Based Push Day Routine",
    },
    {
      day: "Tuesday",
      dow: 2,
      title: "Shakira Fifa",
      id: "fcnDmrtj6Sk",
      description: "Science-Based Pull Day Routine",
    },
    {
      day: "Wednesday",
      dow: 3,
      title: "Phonk",
      id: "zlvQUOtknqY",
      description: "Science-Based Leg Day Routine",
    },
    {
      day: "Thursday",
      dow: 4,
      title: "Remix",
      id: "zlvQUOtknqY",
      description: "Science-Based Shoulders & Arms",
    },
    {
      day: "Friday",
      dow: 5,
      title: "Full Body / Cardio Music",
      id: "tD4HCZe-tew",
      description: "15 Min High Intensity HIIT",
    },
  ];
  ...
  return (
    <div className="xl:grid xl:grid-cols-[1fr_360px] xl:gap-8 items-start animate-fade-in">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-xs text-muted">
                {scheduledTodayCompletedCount} of {habitsForToday.length} complete
              </div>
            </div>
      ...
      {/* Right Column: Workout Videos Sidebar */}
      <aside className="xl:sticky xl:top-8 w-full mt-6 xl:mt-0">
        <div className="card p-5 bg-[#1a1f2e]/60 border border-border-brand-500/10 backdrop-blur-md">
          <h2 className="text-base font-semibold mb-4 flex items-center gap-2">
            Workout Videos 🎬
          </h2>
          <div className="space-y-4 max-h-[780px] overflow-y-auto pr-1 custom-scrollbar">
            {workoutVideos.map((vid) => {
              const isActiveToday = todayDow === vid.dow;
              return (
                <div
                  key={vid.id}
                  className={`p-3 rounded-xl transition-all border ${
                    isActiveToday
                      ? "bg-brand-500/10 border-brand-500/30 shadow-lg shadow-brand-500/5 ring-1 ring-brand-500/20"
                      : "bg-surface-strong/30 border-surface-border/40 hover:border-surface-border"
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {isActiveToday && (
                      <span className="inline-flex items-center gap-1 rounded-full bg-brand-500/20 text-brand-400 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest animate-pulse">
                        🔥 Today's Routine
                      </span>
                    )}
                  </div>

                  <div className="relative aspect-video w-full rounded-lg overflow-hidden shadow-inner border border-surface-border/30 bg-black/20">
                    <iframe
                      className="absolute inset-0 w-full h-full"
                      src={`https://www.youtube.com/embed/${vid.id}`}
                      title={`${vid.day} - ${vid.title}`}
                      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                      allowFullScreen
                    />
                  </div>

                  <div className="mt-2 text-sm font-semibold truncate text-[var(--text)]">
                    {vid.title}
                  </div>
